...
public class ReadWriteLock {
    ...
    public synchronized void unlockReadOnly() {
        ...
        if(readers==0){ //should only notify when there is no readers
        	this.notifyAll();
        }
    }
    ...
    public synchronized void unlockReadWrite() {
        ...
        if(writers==0){ //writers should only be 0 or 1 at this point to unlock, because only 1 writer can be writing
        	this.notifyAll();
        }
    }
}
